...
# Changed eval.py to produce probability maps
import argparse
import os
import torch
import yaml
...
def main():
    ...
    parser.add_argument('--img_root', type=str,
                        help='Root directory for input images when saving visualizations')
    parser.add_argument('--no_metrics', action='store_true',
                        help='Disable metric computation; just run inference + save predictions')
...
class Eval:
    def __init__(self, experiment, args, cmd=dict(), verbose=False):
        ...
        # Inference-only flag: when True we skip validate_measure / gather_measure
        self.inference_only = cmd.get('no_metrics', False)

        # Optional override for image root (for non-ICDAR datasets)
        self.img_root = cmd.get('img_root', None)
        ...
        with torch.no_grad():
            for _, data_loader in self.data_loaders.items():
                ...
                for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
                    ...
                    img_root = (
                        self.img_root
                        or os.path.join('datasets', 'icdar2015', 'test_images')
                    )
                    ...
                    if self.args.get('save_prob_maps', False):
                        self.save_all_spatial_maps(
                            batch,
                            pred,
                            img_root=img_root,  # use same root as above
                            save_gray=self.args.get('save_prob_maps_gray', False)
                        )

                    if (not self.inference_only) and hasattr(self.structure, 'measurer'):
                        try:
                            ...
                            self.logger.warning(
                                f"validate_measure failed, skipping metrics for this batch: {e}"
                            )
                ...
                if (not self.inference_only) and raw_metrics and hasattr(self.structure, 'measurer'):
                    try:
                        ...
                        self.logger.warning(
                            f"gather_measure failed, skipping overall metrics: {e}"
                        )
