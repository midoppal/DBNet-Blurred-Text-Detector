...
SAVE_NPY   = True   # also save float32 .npy maps
# ===============================

os.makedirs(OUT_DIR, exist_ok=True)
device = "cuda" if torch.cuda.is_available() else "cpu"

# ---------- TextHead: MUST MATCH TRAINING / dino_eval.py ----------
...
# ---------- Extract patch features (robust for DINOv2) ----------
@torch.no_grad()
def extract_feats(model, x):
    ...
    if isinstance(out, torch.Tensor):
        ...
        # Prefer patch tokens if available
    ...
    # Many ViTs include a CLS token; detect via grid size
    if hasattr(model, "patch_embed") and hasattr(model.patch_embed, "grid_size"):
        ...
        if N == n_patch + 1:
            ...
            # CLS + extra tokens at the end (e.g., reg tokens)
    ...
    fmap = tokens.reshape(B, h, w, C).permute(0, 3, 1, 2).contiguous()  # [B,C,h,w]
    return fmap
...
def run():
    ...
    total_model_time = 0.0   # time for DINO backbone + head (+ interpolation)
    total_full_time = 0.0    # optional: includes cv2.imread + saving
    n_images = 0
    # ----------------------------------
    
    for p in tqdm(files, ncols=100):
        full_start = time.time()  # start full pipeline timing
        ...
        # ---- DINO model timing (backbone + head + upsample + unletterbox) ----
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        model_start = time.time()

        with torch.no_grad():
            f = extract_feats(model, x)          # (1,C,h',w')
            logits = head(f)                     # (1,1,h',w')
            prob_lr = torch.sigmoid(logits)      # (1,1,h',w')
            ...
            prob = unletterbox_to_original(prob_sq, pads, orig_hw, resized_hw)  # HxW
