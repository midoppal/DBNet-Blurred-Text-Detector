...
# FOLDER_RE = re.compile(r"^length_(\d+)_jitter_(\d+)$", re.I)
FOLDER_RE = re.compile(r"length_(\d+)_jitter_(\d+)", re.I)
...
def main():
    ...
    root = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("eval_results_ic15/google_vision")
    if not root.exists():
    ...
    for sub in sorted(root.iterdir()):
        ...
        length, jitter = int(m.group(1)), int(m.group(2))
        precision, recall, fmeasure = parsed
        rows.append((length, jitter, precision, recall, fmeasure))
    ...
    with out_csv.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["length", "jitter", "precision", "recall", "fmeasure"])
        w.writerows(rows)
